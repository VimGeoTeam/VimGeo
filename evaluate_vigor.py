...
import numpy as np
from dataset.VIGOR_inference import VIGOR
from dataset.CVUSA_512 import CVUSA
from dataset.CVACT import CVACT
from model.MambaGeo_cut_mapping_6144_vigor import MambaGeo
...
os.environ["CUDA_VISIBLE_DEVICES"] = "2,3"  # 2 GPU
...
def train(train_loader, model, criterion, optimizer, epoch, args, train_sampler=None):
    ...
    for i, (images_q, images_k, indexes, _, _ ) in enumerate(train_loader):
...
def validate(val_query_loader, val_reference_loader, model, args):
    ...
    if args.evaluate:
        checkpoint = torch.load(args.save_path + '/model_best.pth.tar', map_location=torch.device('cpu'))
    ...
    # For multi-label support (VIGOR)
    is_multilabel = hasattr(val_query_loader.dataset, 'test_label') and \
                   len(val_query_loader.dataset.test_label.shape) > 1
    if is_multilabel:
        query_labels = []  # Will store list of lists for multi-label
    else:
        query_labels = np.zeros(len(val_query_loader.dataset))  # Single-label
    with torch.no_grad():
        end = time.time()
        ...
        for i, batch in enumerate(val_query_loader):
            if len(batch) == 3:  # (images, indexes, labels)
                images, indexes, labels = batch
            else:  # Handle cases where labels might be packed differently
                images, indexes = batch[:2]
            ...
            if labels is not None:
                if is_multilabel:
                    # For VIGOR: labels is a list of positive reference indices
                    query_labels.extend(labels.cpu().numpy().tolist())
                else:
                    # For CVUSA/CVACT: single label per query
        ...
        hit1, top1 = accuracy(query_features, reference_features, query_labels)
    if args.evaluate:
    ...
    return  top1  # Return both metrics
...
def accuracy(query_features, reference_features, query_labels, topk=[1,5,10]):
    """Computes accuracy metrics for both single-label and multi-label scenarios"""
    ts = time.time()
    N = query_features.shape[0]
    M = reference_features.shape[0]
    topk.append(M//100)  # Add Top-1% metric
    results = np.zeros(len(topk))
    hit1 = 0.0  # Separate counter for Hit@1 (multi-label)
    # Determine if we're in multi-label mode (VIGOR)
    is_multilabel = isinstance(query_labels, list) and \
                   len(query_labels) > 0 and \
                   isinstance(query_labels[0], (list, np.ndarray))
    # Normalize features
    query_features_norm = query_features / np.linalg.norm(query_features, axis=1, keepdims=True)
    reference_features_norm = reference_features / np.linalg.norm(reference_features, axis=1, keepdims=True)
    # Process in chunks to avoid memory issues
    def process_chunk(query_chunk, labels_chunk):
        nonlocal hit1
        similarity = np.matmul(query_chunk, reference_features_norm.T)
        chunk_size = query_chunk.shape[0]
        chunk_results = np.zeros(len(topk))
        for i in range(chunk_size):
            if is_multilabel:
                # Multi-label logic (VIGOR)
                positive_indices = labels_chunk[i]  # All positive reference indices
                primary_positive = positive_indices[0]  # First is main positive
                
                # Hit@1: Check if top prediction is any positive
                top1_pred = np.argmax(similarity[i])
                if top1_pred in positive_indices:
                    hit1 += 1
                
                # Top-k: Based on main positive's ranking
                ranking = np.sum(similarity[i] > similarity[i, primary_positive])
            else:
                # Single-label logic (CVUSA/CVACT)
                ranking = np.sum(similarity[i] > similarity[i, labels_chunk[i]])
                # For single-label, Hit@1 == Top-1
                if ranking < 1:
                    hit1 += 1
            # Update all top-k metrics
            for j, k in enumerate(topk):
                if ranking < k:
                    chunk_results[j] += 1
        return chunk_results
    # Process data in chunks for large datasets
    if N >= 80000:  # VIGOR-scale datasets
        chunk_size = N // 4
        assert N % 4 == 0, "For large N, it must be divisible by 4"
        for split in range(4):
            start = split * chunk_size
            end = (split + 1) * chunk_size
            chunk_results = process_chunk(
                query_features_norm[start:end],
                query_labels[start:end] if not is_multilabel else query_labels[start:end]
            )
            results += chunk_results
    else:  # Smaller datasets (CVUSA/CVACT)
        results = process_chunk(query_features_norm, query_labels)
    # Normalize results
    results = results / N * 100
    hit1 = hit1 / N * 100
    # Print comprehensive results
    print(
        f'Hit@1:{hit1:.6f}%, Top-1:{results[0]:.6f}%, '
        f'Top-5:{results[1]:.6f}%, Top-10:{results[2]:.6f}%, '
        f'Top-1%:{results[-1]:.6f}%, Time:{time.time()-ts:.6f}s'
    )
    
    return hit1, results[0]  # Return (Hit@1, Top-1)
