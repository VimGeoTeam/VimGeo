...
class VIGOR(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index, debug=False):
        if 'train' in self.mode:
            ...
            return img_query, img_reference, torch.tensor(idx), torch.tensor(idx), torch.tensor(self.train_delta[idx, 0])
        elif 'test_query' in self.mode:
            img_query = Image.open(self.test_list[index])
            img_query = self.transform_query(img_query)
            return img_query, torch.tensor(index), torch.tensor(self.test_label[index])  # 返回全部4个标签
        # elif 'test_query' in self.mode:
        #     img_query = Image.open(self.test_list[index])
        #     img_query = self.transform_query(img_query)
